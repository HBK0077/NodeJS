...
window.addEventListener("DOMContentLoaded",async()=>{
   try{
  const response=await axios.get("http://localhost:8081/show-details")
...
function deleteProduct(key){
    axios.delete(`http://localhost:8081/delete-details/${key}`)
    .then((resource)=>{
...
btn.addEventListener("click",(e)=>{
   ...
   axios.post("http://localhost:8081/add-details",my_obj)
   .then((response)=>{
